...
Also note that we kept the number of MCMC iterations low, so it is possible to reproduce
all of the above examples quickly. You should increase this number when attempting your
own analyses.
