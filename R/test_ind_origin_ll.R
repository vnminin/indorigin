...
did not match: the former were ignored in the analysis.')
  ...
  for (i in 1:length(postProbs)){
    ...
    #test <- manyTreesConvolveTest(mcmcOut[,"treeIndex"], inputTrees, treeTraits, mcmcOut[,"lambda01"], mcmcOut[,"lambda10"], 0, testThreshold, "posterior")
  }
...
testIndOrigin_new = function(inputTrees, rootDist=c(0,1), traitData, initLambda01, initLambda10, priorAlpha01, priorBeta01, priorAlpha10, priorBeta10, mcmcSize, mcmcBurnin, mcmcSubsample, mcSize, testThreshold=0, average = FALSE){
  ...
  ## Run Gibbs sampler that iterates between drawing from the full conditional of missing data
  ## and drawing from the full conditional of model parameters (rates 0->1 and 1->0)
  cat("running Gibbs sampler and calculating posterior and prior probabilities", "\n")
  mcmcRun = twoStatePhyloGibbsSamplerAndConvolveStuff(treeEdges, dim(treeEdges), treeBranchLengths, rootDist, treeTraits, initLambda01, initLambda10, priorAlpha01, priorBeta01, priorAlpha10, priorBeta10, mcmcSize, mcmcBurnin, mcmcSubsample, mcSize, testThreshold, TRUE, TRUE, TRUE, average)
