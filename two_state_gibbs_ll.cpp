...
// [[Rcpp::export]]
List twoStatePhyloGibbsSamplerAndConvolveStuff(IntegerVector treeEdges, IntegerVector cubeDims, NumericMatrix branchLengths,
                                        NumericVector rootDist, IntegerMatrix tipStates, double initial_lambda_01, 
                                        double initial_lambda_10, double prior_alpha_01, double prior_beta_01, 
                                        double prior_alpha_10, double prior_beta_10, int mcmcSize, int mcmcBurnin, 
                                        int mcmcSubsample, int mcSize, int n_max, bool gains, bool postSwitch, bool priorSwitch, bool average){
  ...
  std::vector<PhyTree *> vecTrees;  
  for( int i=0; i<cubeDims[2]; i++){
    PhyTree* temp = new PhyTree(cubeTreeEdges.slice(i), vecBranchLengths[i], vecTipStates[i]);
    vecTrees.push_back(temp);  
  }
  ...
  for (int i=0; i<mcmcSize; i++){
    ...
    if (i > mcmcBurnin-1){
      ...
      if(stepCount==mcmcSubsample){
        ...
        // put if statement here -- > put flag so can run MCMC without calculating posterior stuff
        stepCount=0;
      }
    }
  }
  
  arma::vec postProb(numSavedSamples);
  if (postSwitch == true){  
    for (int i= 0; i<numSavedSamples; i++){
      lambda_01 = mcmcOut(i, 3);
      lambda_10 = mcmcOut(i, 4);
      // PhyTree tree(cubeTreeEdges.slice(treeInd), vecBranchLengths[treeInd], vecTipStates[treeInd]);
      if (average == false){
        int treeInd = mcmcOut(i, 1)-1;
        postProb(i) = arma::accu(arma::exp(convolveTree(*vecTrees[treeInd], n_max, lambda_01, lambda_10, rootDist[0], "posterior", gains)));
      } else {
        postProb(i) = arma::accu(averageTreesConvolve(vecTrees, n_max, lambda_01, lambda_10, rootDist[0], "posterior", gains))/numTrees;
      }
    }
  }       
        
  // use MCMC output from prevous function
  // it's ok to have two "for" loops
  
  arma::vec priorProb(mcSize);
  if (priorSwitch == true){
    for (int i=0; i<mcSize; i++){
      //sample a tree from the list uniformly at random
      // sample CTMC rates
      lambda_01 = ::Rf_rgamma(prior_alpha_01, 1/(prior_beta_01));
      lambda_10 = ::Rf_rgamma(prior_alpha_10, 1/(prior_beta_10));
      if (average == false){
      int treeInd = sampleOnce(unifWeights, as<double>(runif(1)));
      //PhyTree tree(cubeTreeEdges.slice(treeInd), vecBranchLengths[treeInd], vecTipStates[treeInd]);
      priorProb(i) = arma::accu(arma::exp(convolveTree(*vecTrees[treeInd], n_max, lambda_01, lambda_10, rootDist[0], "prior", gains)));
      } else {
      priorProb(i) = arma::accu(averageTreesConvolve(vecTrees, n_max, lambda_01, lambda_10, rootDist[0], "prior", gains))/numTrees;
      }
  ...
  //return mcmcOut;
  if (priorSwitch == true && postSwitch == true){
    return Rcpp::List::create(Rcpp::Named( "mcmcOut" ) = mcmcOut, Rcpp::Named( "postProb" ) = postProb, Rcpp::Named("priorProb") = priorProb);
  } else if (priorSwitch == false && postSwitch == true){
    return Rcpp::List::create(Rcpp::Named( "mcmcOut" ) = mcmcOut, Rcpp::Named( "postProb" ) = postProb);
  } else if (postSwitch == false && priorSwitch == true){
    return Rcpp::List::create(Rcpp::Named( "mcmcOut" ) = mcmcOut, Rcpp::Named("priorProb") = priorProb);
  } else {
    return Rcpp::List::create(Rcpp::Named( "mcmcOut" ) = mcmcOut);
  }
}
